#Runs, takes eons to finish, but it does finish. Accuracy is terrible.
...
def create_model(lstm_units=50, dropout_rate=0.2, optimizer='adam'):
    model = Sequential()
    model.add(LSTM(lstm_units, return_sequences=True, input_shape=(60, 11)))  # Adjusted input shape to match number of features
...
def stock_market_analysis(stock_ticker, start_date="2010-01-01", end_date="2024-01-01", test_ratio=0.2, benchmark_ticker='VOO'):
    ...
    features = ['Close', 'Volume', 'RSI', 'MACD', 'MACD_Signal', 'BB_High', 'BB_Low', 'Rate of Return', '50_MA', '100_MA', 'Benchmark_Close']
    ...
    model = KerasRegressor(model=create_model, lstm_units=50, dropout_rate=0.29206627614880376, optimizer='adam', epochs=10, batch_size=32, verbose=0)
    ...
    final_model = create_model(lstm_units=best_params['lstm_units'], dropout_rate=best_params['dropout_rate'], optimizer=best_params['optimizer'])
    ...
    final_model.fit(x_train, y_train, batch_size=32, epochs=30, validation_split=0.2, callbacks=[early_stopping, model_checkpoint])
    ...
    valid = pd.DataFrame(index=prediction_dates, data=predictions, columns=[f'Predictions_{feature}' for feature in features])
